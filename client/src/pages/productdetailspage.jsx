...
const ProductDetails = (props) => {
    ...
    useEffect(() => {
        const fetchdetails = async () => {
            try {
                const res = await axios.get(`https://green-pitch.vercel.app//productdetails/${id}`);
    ...
    const ToCart = async () => {
        ...
        if (Token) {
            //console.log('hm')
            try {
                const res = await axios.post('https://green-pitch.vercel.app//addcart',
                    {
    ...
    const toWish = async () => {
        try {
            ...
            if (token) {
                const res = await axios.post(`https://green-pitch.vercel.app//addwish/${id}`, {},
                    {
                        headers: { Authorization: `Bearer ${token}` }
                    }
                )
                ...
                {details && (
                    <div className='flex flex-col mb-20 lg:flex-row items-start justify-center px-4 lg:px-0'>
                        <div className='w-full lg:w-2/5 mt-10 lg:mt-35 mb-6 lg:mb-0 flex justify-center'>
                            <img
                                src={`https://green-pitch.vercel.app/${details.brandimg}`}
                                alt="productimage"
