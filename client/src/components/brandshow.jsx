...
const BrandShow = (props) => {
    ...
    const [loading,setLoading]=useState(true)
     ...
     useEffect(()=>{
        const fetch=async()=>{
            try{
                const brands=await axios.get('https://green-pitch.vercel.app//brand')
               ...
               variants={{
                 ...
                 <Link to={`/productlistbybrand/${item._id}`}>
                        <motion.div key={item._id} className='shadow-md rounded-md  m-5 w-40 h-40 flex flex-col items-center cursor-pointer transition-transform ease-in-out
                            ...
                            <img src={`https://green-pitch.vercel.app/${item.brandimg}`} alt={item.brandname} height={200} width={200} className='max-w-full max-h-[100px] p-4 object-contain'/>
